...
function sydney_custom_header_setup() {
	$current_document_url = $_SERVER['REQUEST_URI'];
	$current_document_url = substr($current_document_url, 14);
	$current_document_url = substr($current_document_url, 0, -1);

	add_theme_support( 'custom-header', apply_filters( 'sydney_custom_header_args', array(
		'default-image'          => get_template_directory_uri() . '/images/'.$current_document_url.'.jpg',
